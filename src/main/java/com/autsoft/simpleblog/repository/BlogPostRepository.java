...
public interface BlogPostRepository extends JpaRepository<BlogPost, Long> {
    ...
    @Query(value = "SELECT DISTINCT bp.id FROM category c JOIN category_tags ct ON c.id = ct.category_id JOIN tag ON tag.id = ct.tag_id JOIN blog_post_categories bpc ON bpc.category_id = ct.category_id JOIN blog_post bp ON bp.id = bpc.blog_post_id WHERE tag.label=?1",
    nativeQuery = true)
